...
class HomeController extends BaseController
{
    ...
    public function article($slug)
    {
		...
		/*$post = DB::table('posts')
			...
			->select('posts.id', 'categories.redirect_url')
            ->first();*/
		
		//Check If Redirection needed
		$redirect_type = $post->category->redirect_type;
		if($redirect_type=='Link' && !empty($post->category->redirect_url) && filter_var($post->category->redirect_url, FILTER_VALIDATE_URL)){
			return redirect($post->category->redirect_url, 301);
		}
		...
		if($redirect_type == 'Splash Page'){
			 $this->data['redirect_url'] = $post->category->redirect_url;
			 $this->data['splash_page_content'] = $post->category->splash_page_content;
			 return view('splash', $this->data);
		}
    ...
    public function categoryRss($slug)
    {
        ...
        foreach ($posts as $post) {
            ...
            if ($post->type == Posts::TYPE_SOURCE) {
                if ($settings_general->include_sources == 1) {
					$temp = strip_tags($post->description);
					$temp_img = $post->featured_image;
					$temp = "<img src='$temp_img' />".$temp;
                    $feed->item(['title' => html_entity_decode($post->title),
                            'description|cdata' => substr($temp, 0, 9999),
                            'link' => URL::to($post->slug),
                            'guid' => $post->id,
                            'author' => $author->name,
