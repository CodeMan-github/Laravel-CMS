...
@section('extra_css')
    <title>{{trans('messages.tag')}} : {{$tag->title}}</title>
    ...
    <meta property="twitter:title" content="Tag : {{$tag->title}}"/>
    ...
    <meta property="og:title" content="Tag : {{$tag->title}}"/>
    ...
    <div class="container main-wrapper">
        <div class="main-content mag-content clearfix">
            ...
            <div class="row blog-content">
                <div class="col-md-8">
                    <h3 class="tag-title">{{trans('messages.tag')}}: <span>{{$tag->title}}</span></h3>

                    @if(sizeof($posts) == 0)
                        <h4>{{trans('messages.no_results_found_for_this_tag')}} - {{$tag->title}}</h4>

                        @if(!empty($ads[\App\Ads::TYPE_BETWEEN_TAG_INDEX]))
                            <div class="widget adwidget">
                                {!! $ads[\App\Ads::TYPE_BETWEEN_TAG_INDEX]->code !!}
                            </div>
                        @endif
