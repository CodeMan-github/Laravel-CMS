...
@section('extra_css')
    <title>{{trans('messages.category')}} : {{$category->title}}</title>

    <meta name="keywords" content="{{$settings_seo->seo_keywords}}">
    <meta name="description" content="{{\Illuminate\Support\Str::limit(trim(strip_tags($category->seo_description)),300)}}">

    <!--Twitter Card-->
    <meta property="twitter:card" content="summary"/>
    <meta property="twitter:site" content="{{$settings_social->twitter_handle}}"/>
    <meta property="twitter:title" content="Category : {{$category->title}}"/>
    <meta property="twitter:description"
          content="{{\Illuminate\Support\Str::limit(trim(strip_tags($category->seo_description)),300)}}"/>
    ...
    <meta property="og:title" content="Category : {{$category->title}}"/>
            ...
            <div class="row blog-content">
                <div class="col-md-8">
                    <h3 class="tag-title"><span class="social-rss">
                            <a href="/rss/{{$category->slug}}" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="RSS">
                                <i class="fa fa-rss"></i>
                            </a>
                        </span> {{trans('messages.category')}} : <span>{{$category->title}}</span></h3>

                    @if(sizeof($posts) == 0)
                        <h4>{{trans('messages.no_posts_for_category')}} - {{$category->title}}</h4>

                        @if(!empty($ads[\App\Ads::TYPE_BETWEEN_CATEGORY_INDEX]))
                            <div class="widget adwidget">
                                {!! $ads[\App\Ads::TYPE_BETWEEN_CATEGORY_INDEX]->code !!}
                            </div>
                        @endif
                    @endif
